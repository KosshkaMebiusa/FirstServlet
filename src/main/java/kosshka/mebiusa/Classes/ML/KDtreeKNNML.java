...
import static kosshka.mebiusa.Classes.ML.MachineLearning.makeDataset;

/**
 * Created by kosshka_mebiusa on 23.04.16.
 */
public class KDtreeKNNML extends MLAlgorithm {

    List<Weather> weatherList;
    int weatherItem;

    public KDtreeKNNML(List<Weather> weatherList, int weatherItem){
    ...
    public void bestParametrs(){
        ...
        for (int iN = min; iN < max; iN++)
            for (int ik = min; ik < max; ik++) {
                ...
                Classifier classifier = new KDtreeKNN(ik);
